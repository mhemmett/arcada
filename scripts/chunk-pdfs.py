...
metadata (DOI, year, journal, author, instrument links). Otherwise metadata
is inferred from the PDF itself.
...
def resolve_meta(doc, pdf_path: Path, lookup: dict) -> dict:
    ...
    if match:
        return {
            ...
            "tags":               match.get("tags", []),
            "linked_instruments": match.get("linked_instruments", []),
        }
...
def process_pdf(pdf_path: Path, lookup: dict) -> list[dict]:
    ...
    for i, chunk_text in enumerate(chunks):
        ...
        result.append({
            ...
            "linked_instruments": doc_meta.get("linked_instruments", []),
            "chunk_index":        i,
